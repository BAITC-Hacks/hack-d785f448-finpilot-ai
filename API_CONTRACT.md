...
Без ключа: mode=replay, model=null. answer._source=template — шаблон из фактов, replay — сохранённый ответ. Успешный живой ответ: mode=live и _source=live:<model>. При сбое провайдера возвращается шаблон с пометкой ошибки и mode=replay. Health показывает настроенный режим; для проверки live проверять именно объяснение. Replay не является проверкой live-модели.

## Ошибки

JSON: {"error":"сообщение"}. HTTP 400 — неверный JSON/параметр; 403 — POST с чужим Origin; 404 — GID или путь отсутствует; 409 — конфликт решения/устаревший черновик; 503 — база недоступна или запись не выполнена; 500 — внутренняя ошибка. При настроенной базе ошибка записи не считается успешным прогоном. POST ограничен 16 КБ.
